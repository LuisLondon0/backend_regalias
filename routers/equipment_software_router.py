...
@router.get("/equipment_softwares/project/{project_id}", response_model=EquipmentSoftwareResponse)
def get_equipment_software_by_project_id(project_id: int):
    equipment_software = service.get_equipment_software_by_project_id(project_id)
    if equipment_software is None:
        raise HTTPException(status_code=404, detail="Equipment software not found")
    return equipment_software